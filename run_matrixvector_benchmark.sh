...
# Matrix-Vector Example Custom Benchmark Runner
# Compares TornadoVM-generated kernels vs user-provided OpenCL kernels

SIZE="${1:-2048}"
GENERATED_KERNEL="${2:-kernels/matrixvector_generated.cl}"
CUSTOM_KERNEL="${3:-kernels/matrixvector_custom.cl}"
...
java --enable-preview @tornado-argfile \
  ...
  uk.ac.manchester.tornado.examples.compute.custom.MatrixVectorExampleCustomBenchmark \
  "${SIZE}" \
  "${GENERATED_KERNEL}" \
  "${CUSTOM_KERNEL}"
