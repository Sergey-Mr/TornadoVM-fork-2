                                  ...
                                  __private  int    localWorkGroupSize)
{
    ...
    int rowBaseW = (group << 13) + HEADER; // same as (group*8192 + 4)

    float sum = 0.0f;

    // CLEAN: explicit k loop instead of manual i_10 / i_22 updates
    for (int k = lid; k < N; k += WG)
    {
        float wv = w_f[rowBaseW + k];
        float xv = x_f[HEADER + k]; // original: i_16 = i_10 + 4
    ...
    // Reduction still uses the same halving pattern
    int offset = 16;
    for (; offset >= 1; )
    {
        if (lid < offset)
