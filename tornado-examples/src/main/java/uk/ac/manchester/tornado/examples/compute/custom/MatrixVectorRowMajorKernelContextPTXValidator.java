...
/**
 * PTX Validator for KernelContext-based Matrix-Vector Row Major kernels.
 * Compares outputs from two PTX kernels to verify they produce identical results.
 *
 * Usage: MatrixVectorRowMajorKernelContextPTXValidator <kernel1.ptx> <kernel2.ptx> [inputDim] [outputDim]
 *
 * Default: inputDim=8192, outputDim=2048
 *
 * NOTE: This validator is for KernelContext-based PTX kernels which have extra parameters:
 *   (kernel_context, context_unused, x, hb, w, n, d, localWorkGroupSize)
 */
public class MatrixVectorRowMajorKernelContextPTXValidator {
    ...
    private static void runKernel(String kernelPath, FloatArray input, FloatArray output,
        ...
        // KernelContext placeholder arrays
        LongArray kernelContext = new LongArray(64);
        LongArray contextUnused = new LongArray(64);

        // Parameters match KernelContext PTX signature:
        // (kernel_context, context_unused, x, hb, w, n, d, localWorkGroupSize)
        AccessorParameters accessors = new AccessorParameters(8);
        accessors.set(0, kernelContext, Access.READ_ONLY);
        accessors.set(1, contextUnused, Access.READ_ONLY);
        accessors.set(2, input, Access.READ_ONLY);
        accessors.set(3, output, Access.WRITE_ONLY);
        accessors.set(4, weights, Access.READ_ONLY);
        accessors.set(5, Long.valueOf(inputDim), Access.NONE);
        accessors.set(6, Long.valueOf(outputDim), Access.NONE);
        accessors.set(7, Long.valueOf(LOCAL_WORK_GROUP_SIZE), Access.NONE);
    ...
    public static void main(String[] args) throws TornadoExecutionPlanException {
        if (args.length < 2) {
            System.out.println("Usage: MatrixVectorRowMajorKernelContextPTXValidator <kernel1.ptx> <kernel2.ptx> [inputDim] [outputDim]");
            System.out.println("  Default inputDim: 8192");
            System.out.println("  Default outputDim: 2048");
            System.out.println();
            System.out.println("NOTE: This validator is for KernelContext-based PTX kernels.");
            System.out.println("Example:");
            System.out.println("  MatrixVectorRowMajorKernelContextPTXValidator kernels/ptx/matvec_generated.ptx kernels/ptx/matvec_custom.ptx 8192 2048");
            System.exit(1);
        }
        ...
        System.out.println("=== Matrix-Vector Row Major KernelContext PTX Validator ===");
